...
export default function AddressSelector() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data: Record<string, RegionData> = await response.json();

        const provincesList: AddressData = {};
        Object.values(data).forEach((region: RegionData) => {
  ...
  useEffect(() => {
    if (selectedProvince && addressData[selectedProvince]) {
      setCities(
        Object.keys(addressData[selectedProvince].municipality_list).sort()
      );
      setSelectedCity("");
      setBarangays([]);
    } else {
  ...
  useEffect(() => {
    if (
      selectedCity &&
      addressData[selectedProvince]?.municipality_list[selectedCity]
    ) {
      setBarangays(
        addressData[selectedProvince].municipality_list[
          selectedCity
        ].barangay_list.sort()
      );
    } else {
